...
export default function ContactPage() {
  ...
  const onSubmit = form.handleSubmit(async (values) => {
    try {
      ...
      toast.success(isFr ? "Votre message a bien ete envoye." : "Your message was sent successfully.");
      form.reset();
      router.push(isFr ? "/fr/thank-you?type=contact" : "/thank-you?type=contact");
    } catch (error) {
      console.error(error);
      toast.error(isFr ? "Nous n'avons pas pu envoyer votre message. Veuillez reessayer." : "We could not send your message. Please try again.");
    } finally {
      setSubmitting(false);
    }
  });
  ...
  const errorText: Partial<Record<keyof ContactFormInput, string>> = isFr
    ? {
    ...
    <main className="mx-auto max-w-7xl px-4 py-10 sm:py-12 space-y-8">
      <section className="overflow-hidden rounded-[2rem] border border-border/50 bg-[linear-gradient(150deg,rgba(255,255,255,0.82),rgba(248,238,221,0.92))] shadow-[0_30px_80px_rgba(0,0,0,0.08)]">
        <div className="grid gap-0 lg:grid-cols-[1.05fr_0.95fr]">
          <div className="flex flex-col justify-center p-8 sm:p-10 lg:p-14">
            <p className="ui-eyebrow mb-3">{isFr ? "Commencez votre voyage" : "Begin Your Journey"}</p>
            <h1 className="ui-section-title max-w-xl">Contact</h1>
            <p className="ui-prose mt-4 max-w-xl text-base sm:text-lg">
              {isFr
                ? "Dites-nous ce que vous souhaitez reserver a Marrakech et nous vous repondrons rapidement."
                : "Tell us what you want to book in Marrakech and we will get back to you quickly."}
            </p>

            <div className="mt-8 grid gap-3 sm:grid-cols-3">
              {[
                { label: isFr ? "Reponse rapide" : "Fast reply", value: isFr ? "Sous 24h" : "Within 24h" },
                { label: isFr ? "Equipe locale" : "Local team", value: isFr ? "Basee a Marrakech" : "Marrakech based" },
                { label: isFr ? "Assistance" : "Support", value: isFr ? "7 jours sur 7" : "7 days a week" },
              ].map((item) => (
          ...
          <div className="relative min-h-80 lg:min-h-full">
            ...
            <div className="absolute inset-x-0 bottom-0 p-6 sm:p-8 text-white">
              <p className="text-[10px] font-bold uppercase tracking-[0.24em] text-white/80">{isFr ? "Esprit Marrakech" : "Marrakech Vibes"}</p>
              <p className="mt-2 max-w-md text-sm text-white/85">
                {isFr
                  ? "Textures riches, lumiere chaude et esprit de la ville rouge donnent le ton a votre prochaine experience."
      ...
      <div className="grid gap-6 md:grid-cols-2 md:gap-8">
        <form onSubmit={onSubmit} className="ui-surface space-y-3 p-5 sm:p-6">
          <p className="ui-eyebrow mb-2">{isFr ? "Envoyer un message" : "Send a message"}</p>
          ...
          />
          {errors.fullName && <p className="text-xs text-red-600">{errorText.fullName || errors.fullName.message}</p>}
        ...
        <div className="ui-surface-soft space-y-4 p-5 sm:p-6">
          ...
          <p className="text-zinc-700">{isFr ? "Telephone" : "Phone"}: +212 771-444668</p>
          <p className="text-zinc-700">{isFr ? "Adresse: Marrakech, Maroc" : "Address: Marrakech, Morocco"}</p>
