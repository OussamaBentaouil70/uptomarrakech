...
import { getLocaleFromPathname, stripLocalePrefix, localizePath } from "@/lib/locale";
...
export function SiteBreadcrumbs() {
  ...
  if (!pathname || stripLocalePrefix(pathname) === "/") return null;

  const segments = stripLocalePrefix(pathname).split("/").filter(Boolean);
  if (!segments.length) return null;

  const crumbs = segments.map((segment, index) => {
    const href = localizePath(`/${segments.slice(0, index + 1).join("/")}`, locale);
  ...
  return (
    <nav aria-label="Breadcrumb" className="border-b border-border/40 bg-background/95 backdrop-blur-sm">
      <div className="mx-auto flex w-full max-w-7xl items-center gap-2 overflow-x-auto px-4 py-3 text-xs sm:text-sm">
        <Link href={localizePath("/", locale)} className="whitespace-nowrap text-muted-foreground hover:text-foreground transition-colors">
