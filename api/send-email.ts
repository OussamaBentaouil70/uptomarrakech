...
function buildAdminHtml(payload: MailFormPayload, logoUrl: string): string {
...
function buildClientHtml(name: string, logoUrl: string, siteUrl: string): string {
  const primaryColor = "#C39A5A";
  const textColor = "#1F1B16";
  const bgColor = "#F8F2E8";
...
export default async function handler(req: any, res: any) {
  ...
  const smtpSecureRaw = process.env.SMTP_SECURE?.toLowerCase();
  ...
  if (!smtpHost || !smtpPortRaw || !smtpUser || !smtpPass || !fromEmail || !adminEmail) {
    return res.status(500).json({
      success: false,
      message: "Missing SMTP configuration. Set SMTP_HOST, SMTP_PORT, SMTP_USER, SMTP_PASS, FROM_EMAIL, ADMIN_EMAIL.",
    });
  }

  const smtpPort = Number.parseInt(smtpPortRaw, 10);
  if (!Number.isFinite(smtpPort)) {
    return res.status(500).json({ success: false, message: "Invalid SMTP_PORT value." });
  }
  ...
  const payload: MailFormPayload = {
    formType: (body as any)?.formType === "contact" ? "contact" : "reservation",
    contact_name: cleanText((body as any)?.contact_name || ""),
    contact_email: cleanText((body as any)?.contact_email || ""),
    contact_phone: cleanText((body as any)?.contact_phone || ""),
    service_type: cleanText((body as any)?.service_type || "General inquiry"),
    preferred_date: cleanText((body as any)?.preferred_date || ""),
    preferred_time: cleanText((body as any)?.preferred_time || ""),
    number_of_persons: typeof (body as any)?.number_of_persons === "number" ? (body as any).number_of_persons : undefined,
    item_slug: cleanText((body as any)?.item_slug || ""),
    category_type: cleanText((body as any)?.category_type || ""),
    message: cleanText((body as any)?.message || ""),
  };

  if (!payload.contact_name || !payload.contact_email || !payload.message) {
    return res.status(422).json({ success: false, message: "Name, valid email and message are required" });
  }
  ...
  try {
    ...
    return res.status(500).json({ success: false, message: "Mailer Error: Failed to send email" });
  }
}
